...
from ampf.base import BaseAsyncBlobStorage, Blob, KeyNotExistsException
...
async def storage(gcp_async_factory: GcpAsyncFactory) -> BaseAsyncBlobStorage:  # type: ignore
    storage = gcp_async_factory.create_blob_storage("test_collection")
...
async def test_update_transactional_one_thread(storage: BaseAsyncBlobStorage):
    ...
    async def update_func(b: Blob) -> Blob:
...
async def test_update_transactional_two_threads(storage: BaseAsyncBlobStorage):
    ...
    async def update_func1(b: Blob) -> Blob:
    ...
    async def update_func2(b: Blob) -> Blob:
...
async def create_func(name: str) -> Blob:
...
async def test_upsert_transactional_concurrent_creation(storage: BaseAsyncBlobStorage):
    ...
    async def create_func1(name: str) -> Blob:
        await asyncio.sleep(0.1)  # Simulate some processing delay
        return Blob(name=name, content=b"created1")

    async def create_func2(name: str) -> Blob:
