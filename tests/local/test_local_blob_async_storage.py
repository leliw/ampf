...
async def test_upload_and_download(storage: LocalAsyncBlobStorage):
    ...
    blob = Blob[SampleMetadata](name="test_blob", content_type="text/plain", metadata=metadata, content=b"Hello, World!")
...
async def test_list_blobs(storage: LocalAsyncBlobStorage):
    blob1 = Blob[SampleMetadata](name="item1", content_type="application/json", metadata=SampleMetadata(name="first", version=1), content=b"{}")
    blob2 = Blob[SampleMetadata](name="item2", content_type="application/json", metadata=SampleMetadata(name="second", version=2), content=b"{}")
...
async def test_delete_blob(storage: LocalAsyncBlobStorage):
    blob = Blob[SampleMetadata](name="todelete", content_type="text/plain", metadata=SampleMetadata(name="to delete", version=1), content=b"delete me")
    await storage.upload_async(blob)
...
async def test_content_type_affects_file_extension(storage: LocalAsyncBlobStorage):
    blob = Blob[SampleMetadata](
        name="typed_blob", content_type="application/json", metadata=SampleMetadata(name="json test", version=3), content=b'{"foo": "bar"}'
    )
    await storage.upload_async(blob)
...
async def test_update_transactional_two_threads(storage: LocalAsyncBlobStorage):
    ...
    updated_blob = await storage.download_async("test_blob")
    assert updated_blob.content == b"initial_data_updated2_updated1" or updated_blob.content == b"initial_data_updated1_updated2"
