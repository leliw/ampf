...
class ApiTestClient(TestClient):
    def _assert_response(self, response: httpx.Response, status_code: int | None = None) -> None:
    ...
    def get(self, url: httpx._types.URLTypes | Path, status_code: int | None = None, **kwargs) -> httpx.Response:
    ...
    def get_typed[T: BaseModel](
        self, url: httpx._types.URLTypes | Path, status_code: int, ret_clazz: Type[T], **kwargs
    ) -> T:
    ...
    def post(self, url: httpx._types.URLTypes | Path, status_code: int | None = None, **kwargs) -> httpx.Response:
        if isinstance(url, Path):
            url = str(url)
    ...
    def put(self, url: httpx._types.URLTypes | Path, status_code: int | None = None, **kwargs) -> httpx.Response:
        if isinstance(url, Path):
            url = str(url)
        self._prepare_parameters(kwargs)
    ...
    def patch(self, url: httpx._types.URLTypes | Path, status_code: int | None = None, **kwargs) -> httpx.Response:
        if isinstance(url, Path):
            url = str(url)
        self._prepare_parameters(kwargs)
    ...
    def delete(self, url: httpx._types.URLTypes | Path, status_code: int | None = None, **kwargs) -> httpx.Response:
        if isinstance(url, Path):
            url = str(url)
